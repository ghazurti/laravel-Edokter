...
namespace App\Http\Livewire\Component\AwalParu;
...
class Form extends Component
{
    ...
    {
        return view('livewire.component.awal-paru.form');
    }
    ...
    public function updatedNoRawat()
    {
        try {
            $this->resetInput();
            $data = DB::table('penilaian_medis_ralan_paru')
    ...
    public function simpan()
    {
        ...
        try {
            if ($this->editMode) {
                DB::table('penilaian_medis_ralan_paru')
                    ->where('no_rawat', $this->no_rawat)
                    ->update(Arr::except($data, ['no_rawat', 'kd_dokter']));

                $this->alert('success', 'Data berhasil diubah');
            } else {
                DB::table('penilaian_medis_ralan_paru')->insert($data);
        ...
        try {
            ...
            if ($data) {
                $this->confirm('Apakah anda yakin ingin menghapus data ini?', [
                    'onConfirmed'       => 'hapusMedisRalanParu',
