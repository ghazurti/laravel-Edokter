...
namespace App\Http\Livewire\Component\AwalOrthopedi;
...
class Form extends Component
{
    ...
    public $kepala, $thoraks, $abdomen, $ekstremitas, $genetalia, $columna, $muskulos;
    public $lainnya, $ket_lokalis;
    public $lab, $rad, $pemeriksaan;
    public $diagnosis, $diagnosis2, $permasalahan, $terapi, $tindakan, $edukasi;

    protected $listeners = ['hapusMedisRalanOrthopedi' => 'hapus'];
    ...
    {
        return view('livewire.component.awal-orthopedi.form');
    }
    ...
    public function updatedNoRawat()
    {
        try {
            $this->resetInput();
            $data = DB::table('penilaian_medis_ralan_orthopedi')
    ...
    public function simpan()
    {
        ...
        try {
            if ($this->editMode) {
                DB::table('penilaian_medis_ralan_orthopedi')
                    ->where('no_rawat', $this->no_rawat)
                    ->update(Arr::except($data, ['no_rawat', 'kd_dokter']));

                $this->alert('success', 'Data berhasil diubah');
            } else {
                DB::table('penilaian_medis_ralan_orthopedi')->insert($data);
        ...
        try {
            ...
            if ($data) {
                $this->confirm('Apakah anda yakin ingin menghapus data ini?', [
                    'onConfirmed'       => 'hapusMedisRalanOrthopedi',
